...
<body>
	...
	<div class="formbody">
		...
		<table class="tablelist">
			...
			<tbody>
				<tr>
					<td><c:if test="${! empty averagePerdiet}">
							<!-- 为ECharts准备一个具备大小（宽高）的Dom -->
							<div id="recognize" style="height: 400px; width: 800px"></div>

						</c:if> 
						您可以尝试：
						 <c:if test="${! empty recommendCaipuaucList}">
							<c:forEach items="${recommendCaipuaucList}" var="views" varStatus="status">
								<c:if test="${status.index%10==0}">
									<br />
								</c:if>
								<c:if test="${status.last}">
									<a href="Caipu/detail?viewId=${views.viewId}" class="tablelink">${views.viewName}</a>
								</c:if>
								<c:if test="${!status.last}">
									<a href="Caipu/detail?viewId=${views.viewId}" class="tablelink">${views.viewName},</a>
								</c:if>
