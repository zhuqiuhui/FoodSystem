...
public class Apriori {
	...
	final static double MIN_SUPPORT = 0.3;// 最小支持度
	final static double MIN_CONF = 0.1;// 最小置信度
	...
	private static void printConfItemset(List<List<String>> confItemset2) {
		strbuf.append("*********频繁模式挖掘结果***********"+"\r\n"+"<br/>");
		for (int i = 0; i < confItemset2.size(); i++) {
			int j = 0;
			for (j = 0; j < confItemset2.get(i).size() - 3; j++)
			strbuf.append(confItemset2.get(i).get(j) + " ");
			strbuf.append("-->");
			strbuf.append(confItemset2.get(i).get(j++));
			strbuf.append("相对支持度：" + confItemset2.get(i).get(j++));
			strbuf.append("自信度：" + confItemset2.get(i).get(j++) + "\r\n"+"<br/>");
		}
	...
	private static List<List<String>> getRecord1() {
		TxtReader readRecord1 = new TxtReader();
		return readRecord1.getRecord();
	}
	...
	private static List<List<String>> getNextCandidate(
			List<List<String>> cItemset) {
//		System.out.println("上一次备选集"+cItemset);
...
//		System.out.println("-----下一次备选集"+nextItemset);
		return nextItemset;
	}
...
//		System.out.println("---------------输出第一次备选项集------------------");
//		for(List<String> strl:tableList){
